...
// ---- Confidence ring (SVG) ---------------------------------------------
// Animated circular progress. The %-numeral sits at the center, qualitative
// label tucks underneath. Stroke color tracks verdict severity.
...
function ConfidenceRing({
  ...
  React.useEffect(() => {
    const raf = requestAnimationFrame(() => {
      setTimeout(() => setPct(confidence), 160);
    });
      ...
      >
        <defs>
          <linearGradient id="ai-ring-grad" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#0AB7A3" />
            <stop offset="100%" stopColor="#0498C6" />
          </linearGradient>
        </defs>
        <circle
          cx={SIZE / 2}
        ...
        <circle
          ...
          r={R}
          fill="none"
          stroke={verdict === 'clean' ? 'url(#ai-ring-grad)' : `var(--${verdict})`}
          strokeWidth={STROKE}
          strokeLinecap="round"
      ...
      <div className="absolute inset-0 grid place-items-center">
        <div className="text-center">
          ...
          >
            {Math.round(pct)}
            <span
              className="text-caption font-medium ml-0.5"
              style={{ color: 'var(--ink-3)' }}
            >
              %
            </span>
          ...
          <div
            className="font-sans text-eyebrow uppercase tracking-[0.08em] mt-1"
            style={{ color: `var(--${verdict}-ink)`, fontWeight: 700 }}
          >
            {label} confidence
          </div>
        </div>
