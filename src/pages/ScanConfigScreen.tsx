...
const THRESHOLD_AUDIT: AuditEvent[] = [
  { at: '2026-09-01T15:04', actor: 'J. Marlow', changes: [
    { label: 'Not sure · call it Rented at', from: '75%', to: '80%' },
  ] },
...
//
// Amendment (owner call, 2026-09-03): Not sure keeps its tab but its two
// questions are about the finding itself — "call it Rented at" / "call it
// Not rented at" — because an explicit Not sure carries no contradicting or
// matching outcome to declare. Its in-between band reads Unsure, not
// Inconclusive, and it no longer resolves to the org's default type.
//
 ...
 *  rented); false for owner-occupied / second home. Never asked for Not sure —
...
function matchMeaningLine(intent: string, matchIsRented: boolean): string {
  if (intent === 'not-sure')
    return 'Declared Not sure, so there is no baseline to reconcile against. The scan reports only the finding: Rented, Not rented, or Unsure.';
 ...
 *  names the contradicting / matching finding (flips for a rental). Not sure
 *  asks about the finding itself — there is no matching or contradicting
 *  finding to name — so its questions are "call it Rented / Not rented". */
function declareCopy(intent: string, matchIsRented: boolean) {
  if (intent === 'not-sure') {
    return {
      nrLabel: 'Call it Rented at',
      consLabel: 'Call it Not rented at',
      nrHelper: 'How confident the scan must be that the property is rented before we report Rented.',
      consHelper: 'How confident the scan must be that the property is not rented before we report Not rented.',
    };
  }
  ...
  return {
    nrLabel: 'Flag Needs review at',
    consLabel: 'Declare Consistent at',
...
function TypeMeaningList() {
  return (
    <div className="flex flex-col">
      {OCC_INTENTS.map((intent: string) => {
        const matchRented = intentMatchIsRented(intent);
        return (
          <div
            key={intent}
      ...
      {open && (
        <div id={bodyId} className="mt-stack pl-6">
          ...
          <p className="font-sans text-micro m-0 mt-stack mb-1" style={{ color: 'var(--ink-3)' }}>
            Anything that clears neither bar is Inconclusive — Unsure when nothing was declared. What
            a match means, per type:
          </p>
          <TypeMeaningList />
        </div>
      )}
...
function ScanConfigScreen({
  ...
  // "Not sure" no longer resolves to a real type (owner call, 2026-09-03 —
  // reverting the silent Owner-occupied fallback): an explicit Not sure means the
  // scan reports only the finding, so the resolve toggle and its chosen type
  // are gone. `notSureResolve` / `notSureResolveAs` stay on OccConfig so the
  // saved shape is unchanged; this screen just stops editing them.
  ...
  const snapshot = () =>
    JSON.stringify({ defaultIntent, hi, lo, bandRows, bandsCustom, matrix, recurring, staleDays, timeoutOn, timeoutValue, timeoutUnit });
  ...
  return (
    ...
    <div className="pb-section">
      ...
      >
        ...
        {/* Only genuinely declared types get matrix rows. "Not sure" left the
            grid (owner call, 2026-09-03): an explicit Not sure gives no baseline, so there is
            nothing to reconcile, so it produces no Consistent / Inconclusive /
            Needs review verdict and cannot make an address red from here — the
            explainer row below the grid says what happens instead. */}
        {OCC_INTENTS.filter((i: string) => i !== 'not-sure').map((intent: string) => {
        ...
        {/* The Not-sure explainer row — same reading grid as the rows above,
            but inert: no chevron, no pills, no expansion. It states the one
            behaviour there is nothing to configure here for. */}
        <div
          className="border-t border-line grid gap-2 items-baseline py-2.5"
          style={{ gridTemplateColumns: '1.3fr 3fr' }}
        >
          <span className="font-sans text-caption font-medium pl-5" style={{ color: 'var(--ink)' }}>
            Not sure
          </span>
          <span className="font-sans text-caption" style={{ color: 'var(--ink-3)' }}>
            An explicit “Not sure” gives the scan no baseline to reconcile against, so no verdict is
            made. The scan reports only what it found — Rented, Not rented, or Unsure. Set how
            confident it must be under Confidence thresholds below.
          </span>
        </div>
      ...
      >
        ...
        <div className="mt-stack-md">
          {bandsCustom ? (
            (() => {
              // Custom mode: two declare-confidence questions per type. For
              // the declared types they read Needs review / Consistent; for
              // Not sure they ask about the finding itself (Rented / Not
              // rented), because Not sure carries no baseline to reconcile with.
              // The in-between band is never asked either way.
              const rule = bandRows[editIntent];
              const isNotSureTab = editIntent === 'not-sure';
              const matchRented = intentMatchIsRented(editIntent);
              ...
              return (
                <>
                  ...
                  <p className="font-sans text-micro text-ink-3 leading-relaxed m-0 mt-stack-md">
                    If a score clears neither bar, the result is {isNotSureTab ? 'Unsure' : 'Inconclusive'}.
                    TrueOccupancy sets that automatically, so there is nothing for you to configure. These
                    bands apply to {OCC_INTENT_LABEL[editIntent]} only, and only to future scans. Completed
                    scans keep the settings they ran under.
                  </p>
                </>
              );
            })()
